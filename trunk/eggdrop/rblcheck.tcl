...
namespace eval ::rblcheck {
	...
	variable rbls {
		...
		{priority 10 weight 4 rbl OPM.Tornevall.Org desc {Open Proxy} mainurl {http://DNSBL.Tornevall.Org/} checkurl {http://DNSBL.Tornevall.Org/org/scan.php} codes {{127.0.0.2}}}
...
proc ::rblcheck::CheckResult {ip host status origip rbl} {	variable IPs
	if {[info exists IPs($origip)]} {
		...
		if {[info exists ipinfo(checkcount)]} { incr ipinfo(checkcount) } else { set ipinfo(checkcount) 1 }
		# If we didn't return above, it means there wasn't a match in that RBL.
		variable rbls
		if {$ipinfo(checkcount) >= [llength $rbls]} {
